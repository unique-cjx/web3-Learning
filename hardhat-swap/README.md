...
```text
mint: 用户付 token -> 增加 liquidity
burn: 减少 liquidity -> 增加 tokensOwed
collect: tokensOwed -> 用户钱包
```
...
🔥移除该Pool的流动性，把对应的token0/token1转换成“可debit的金额”，本身不转账。

```text
Pool.burn(amount)
  -> 减少 liquidity
  -> 本金 + 已结算手续费进入 tokensOwed
```

**代码部分**
[Pool.sol (line 201)](./contracts/Pool.sol:201)：
...
burn 不直接转账，这样设计可以把“更新仓位/记账”和“真正提款”拆开。

#### Collect （收取手续费）

💵 是真正把钱从 Pool 转给用户的方法，提取的是当前调用者`positions[msg.sender]`中的：`tokensOwed0` 和 `tokensOwed1`

关键代码
[Pool.sol (line 234)](./contracts/Pool.sol:234)：
...
意思是：用户可以请求提取指定数量，但如果请求超过可提取余额，就只提取实际可用的 tokensOwed。
...
```
注意：collect 不会重新计算手续费，也不会减少 liquidity。它只是领取已经记账到 tokensOwed 里的资产。